...
import { VISUAL_LEVELS } from '../render/buildingAssets';
import '../farm.css';
...
function Examples({ all, level }: { all: boolean; level: number }) {
    ...
    const state = useMemo(() => {
        const s = createFarm(1800000000000, 42); s.entities = s.entities.filter(e => e.asset === 'home'); s.entities[0].level = level; s.world.heights.fill(0);
...
export default function RoadLab() {
    const [all,setAll]=useState(false), [level,setLevel]=useState(1);
    return <main className="farm-root farm-road-lab"><header><div><small>XƯỞNG MỸ THUẬT · LỐI TRANG VIÊN</small><h1>{roadName(level)}</h1><p>Lối đi đổi theo ngoại hình nhà chính, giữ nguyên mọi đoạn nối.</p></div><label>Nhà chính cấp <select aria-label="Cấp nhà chính" value={level} onChange={e=>setLevel(Number(e.target.value))}>{VISUAL_LEVELS.map(n=><option key={n} value={n}>{n}</option>)}</select></label><button aria-pressed={all} onClick={()=>setAll(v=>!v)}>{all?'Xem 6 dạng chính':'Xem đủ 16 hướng nối'}</button><a href="?lab=qa">Về sân thử</a></header><Canvas orthographic camera={{position:[14,30,11],zoom:45,near:.1,far:100}} dpr={[1,1.5]} aria-label="Các dạng đường nối liền"><Suspense fallback={null}><Examples all={all} level={level}/></Suspense></Canvas></main>;
}
