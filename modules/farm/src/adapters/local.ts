...
export class LocalFarmRepository implements FarmRepository {
  ...
  async save(next: FarmState, expectedRevision: number | null) {
    ...
    return new Promise<void>((resolve, reject) => {
      ...
      tx.onerror = () => { /* onabort owns failure reporting */ };
      const read = store.get('main');
      read.onsuccess = () => {
        const old = read.result as FarmState | undefined;
        if ((old?.revision ?? null) !== expectedRevision) { problem = new RevisionConflict(); tx.abort(); return; }
...
export class LocalFarmGateway implements FarmSession {
  ...
  static async open(repository: FarmRepository, now = Date.now) {
    ...
    if (!state) {
      ...
      try { await repository.save(state, null); }
      catch (e) { if (!(e instanceof RevisionConflict)) throw e; state = await repository.load(); if (!state) throw e; }
    }
  ...
  private run<T>(job: () => Promise<T>): Promise<T> {
    const next = this.queue.then(job); this.queue = next.catch(() => undefined); return next;
  }
  execute(command: FarmCommand): Promise<CommandResult> {
    return this.run(async () => {
      const result = execute(this.getSnapshot(), command);
      if (result.ok) { await this.repository.save(result.state, this.state.revision); this.state = result.state; }
      return result;
    });
  }
  checkpoint(): Promise<void> {
    return this.run(async () => {
      ...
      if (next.lastWallTime === this.state.lastWallTime) return;
      next.revision = this.state.revision + 1;
      await this.repository.save(next, this.state.revision); this.state = next;
    });
  }
  importSnapshot(imported: FarmState): Promise<void> {
    return this.run(async () => {
      ...
      next.lastWallTime = this.now(); next.revision = this.state.revision + 1;
      await this.repository.save(next, this.state.revision); this.state = next;
    });
  }
}
...
export async function openLocalFarm(): Promise<FarmSession> {
  ...
  try { return await LocalFarmGateway.open(repository); }
  catch (e) { await repository.close(); throw e; }
}
