...
import { ASSETS, CROPS, ITEMS, QUESTS, RECIPES, SPEEDUPS, type AssetId, type CropId, type ItemId, type RecipeId } from '../core/catalog';
...
export function Panels({ s, panel, selected, busy, act, confirm, select, begin, map, plant, fish }: Props) {
    ...
    if (entity) {
        ...
        return <>
  <div className="farm-panel-lead"><CatalogPicture asset={entity.asset} level={entity.level}/><div><b>{ASSETS[entity.asset].kind === 'building' ? `Cấp ${entity.level}/25` : 'Trang trí'}</b><p>{ASSETS[entity.asset].description}</p></div></div>
  ...
  {entity.asset === 'home' && <><div className="farm-stat-grid"><span><b>{s.plots.length}/{plotCap(s)}</b>ruộng</span><span><b>{s.world.owned.length}/{regionCap(s)}</b>khu đất</span><span><b>{homeSpec(s).builderSlots}</b>đội thợ</span></div><p>{homeSpec(s).milestone}</p></>}
