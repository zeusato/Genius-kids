...
export function SubjectMenu({ s, id, busy, tool, details, move, close, act }: { s: FarmState; id: string; busy: boolean; tool: (t: Tool) => void; details: () => void; move: () => void; close: () => void; act: (c: FarmCommand) => Promise<boolean> }) {
  ...
  return <div className="farm-subject-menu" role="group" aria-label={`Thao tác: ${title}`}>
    ...
    <div className="farm-subject-options">
      ...
      {obstacle && <ResourceAction state={s} obstacle={obstacle} busy={busy} act={act}/>}
      {!entity && !obstacle && (!plot || plot.crop && !ripe) && <button onClick={details}><Icon name="eye" size={26}/><span>{plot ? 'Chăm sóc & tăng tốc' : 'Xây cầu'}</span></button>}
    </div>
  </div>;
}
