...
begin;
select pg_advisory_xact_lock(918250001);
create table public.farm_schema_migrations (version text primary key, sha256 text not null, applied_at timestamptz not null default now());
revoke all on public.farm_schema_migrations from public, anon, authenticated;
alter table public.farm_schema_migrations enable row level security;
...
insert into public.farm_schema_migrations(version,sha256) values ('202609180001_farm_cloud','54edcf00a169f97d54b3beb2ff8bc5ac180d5ea4b5aa11f7c9460bc6808287cc');
notify pgrst, 'reload schema';
commit;
select version, applied_at, 'Farm migration applied' as result from public.farm_schema_migrations where version='202609180001_farm_cloud';