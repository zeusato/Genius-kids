...
it('cancels the active generation and bounds retries after model 404',async()=>{
 const fetch=vi.fn(async(_url:string,opts:any)=>opts?.method==='POST'?new Response('',{status:404}):new Response(JSON.stringify({models:[{name:'models/gemini-9.0-flash',supportedGenerationMethods:['generateContent']}]}),{status:200}));vi.stubGlobal('fetch',fetch);
