...
import {ArrowRight,Box,Square,Bot,Check,Crown,Dices,HelpCircle,Leaf,Pause,Play,Plus,RotateCcw,Settings2,ShieldCheck,Sun,UserRound,Users,Volume2,VolumeX,X} from 'lucide-react';
...
export function HorseRace({student,onExit,complete,sound,initialMatch,manageMusic=true}:{student:StudentProfile;onExit:()=>void;complete:(owner:string,s:Match)=>{ok:boolean};sound:Sound;initialMatch?:Match;manageMusic?:boolean}){
 ...
 const [introMatch,setIntroMatch]=useState<string|null>(()=>initialMatch?.revision===0?initialMatch.id:null);
 ...
 const selectView=(view:BoardView)=>{finishIntro();setFreeView(false);setCameraReset(n=>n+1);setPrefs(p=>({...p,view,flat:false}));};
 ...
 useEffect(()=>{
   const s=match;if(!s||s.phase==='over'||paused||motion||dialog||!ready||introActive)return;
 ...
 },[match?.revision,match?.id,paused,motion,dialog,ready,introActive,prefs.fast,makeMove,toss,update]);
 const begin=()=>{
   ...
   const hostColor=party[0].color;const players=party.map((p,i)=>({...p,name:names[i]})).sort((a,b)=>(a.color-hostColor+4)%4-(b.color-hostColor+4)%4);setParty(players);saveParty(student.id,players);setNewGameWarning(false);setMessage('');setRecorded(null);setBotReason('');setHandoff(false);resume();setReady(prefs.flat);const next=newMatch(student.id,players,crypto.randomUUID(),randomInt(players.length));setIntroMatch(prefs.flat||prefs.reduced?null:next.id);update(next);
 };
 const exitToLobby=()=>{finishIntro();worker.current?.terminate();setDraft(loadDraft(student.id));motionRef.current=null;setMotion(null);live.current=null;setMatch(null);setDialog(null);resume();};
 ...
 const moves=match?legalMoves(match):[],needHandoff=!!(prefs.handoff&&active?.kind==='human'&&!handoff&&match?.phase!=='over'),canPlay=!!match&&!paused&&!motion&&!dialog&&ready&&!introActive&&!needHandoff&&active?.kind==='human';
 const boardProps=match?{state:match,intro:introActive,onIntroEnd:finishIntro,motion,paused,reduced:prefs.reduced,light:prefs.light,view:prefs.view,cameraReset,onManualView,onSeats,frame,enabled:canPlay,moves,onPick:makeMove,onFailure:unavailable,onReady}:null;
 const exitCategory=()=>{pauseGame();const s=live.current;if(s&&!saveDraft(s)){setSaved(false);return;}if(s?.phase==='over'&&!complete(student.id,s).ok){setRecorded(false);return;}onExit();};
 return <div className={'hr-root'+(match?' hr-immersive':'')+(introActive?' hr-intro-playing':'')+(match?.phase==='over'?' hr-finished':'')}>
  <header className='hr-header'><BoardGamesBack onBack={exitCategory}/><div className='hr-brand'><span>GENIUS KIDS · CÙNG NHAU CHƠI</span><strong>Cờ Cá Ngựa</strong></div><div className='hr-header-tools'><button className='hr-icon' onClick={sound.toggleSound} aria-label={sound.soundEnabled?'Tắt âm thanh':'Bật âm thanh'}>{sound.soundEnabled?<Volume2 size={20}/>:<VolumeX size={20}/>}</button><button className='hr-icon' onClick={()=>{if(match)pauseGame();setDialog('guide');}} aria-label='Luật chơi'><HelpCircle size={21}/></button><button className='hr-icon' onClick={()=>{if(match)pauseGame();setDialog('settings');}} aria-label='Cài đặt'><Settings2 size={21}/></button></div></header>
