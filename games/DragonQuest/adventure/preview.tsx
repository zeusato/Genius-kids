...
function fixture(region:string,phase:string):Session{
 const config:Config={missionId:region+'-1',grade:2,difficulty:'easy',topic:'mixed',mode:phase==='timed'?'challenge':'story',ai:false},seed=42,map=createMap(config.missionId,seed);
 ...
 const n=phase.startsWith('count')&&countSlot?map.find(n=>n.id===countSlot.nodeId)!:map.find(n=>n.kind===(phase==='boss'?'boss':phase==='portal'?'teleport':phase==='fairy'?'buff':'combat')&&n.index>0)!;
