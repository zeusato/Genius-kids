...
import { MapNode, Session } from './model';
import { TILE_STYLE } from './content';

/** One instanced draw for every tile; one atlas draw for all fifty labels. */
export function BoardTiles({map,session}:{map:MapNode[];session?:Session}){
 ...
 const atlas=useMemo(()=>{
  ...
  map.forEach((n,i)=>{
   ...
   const h=.45,offset=i*4;positions.push(n.x-h,.152,n.z+h,n.x+h,.152,n.z+h,n.x+h,.152,n.z-h,n.x-h,.152,n.z-h);
