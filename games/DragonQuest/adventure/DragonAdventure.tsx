...
export function DragonAdventure({student,complete,controls,onExit,onLegacy,generate,initialSession}:{student:StudentProfile;complete:Completion;controls:Controls;onExit:()=>void;onLegacy:()=>void;generate?:Generate;initialSession?:Session}){
 ...
 const getGeminiApiKey = () => {
  try {
   return localStorage.getItem('mathgenius_gemini_key')
 ...
 const begin=async(forceLocal=false)=>{
  ...
  if(prefs.ai&&!forceLocal&&!transport){
   if(apiKey){
    transport=apiGenerator(apiKey);
   }else{
    setPreparing(false);pending.current=null;
    setMessage('Chưa có API Key Gemini. Vui lòng cài đặt API Key để tạo đề bằng AI.');
    setAiSetup(true);
    return;
   }
  }
  try{const prepared=await prepareQuestions(config,slots,seed,{signal:controller.signal,generate:forceLocal?undefined:transport,budgetMs:60000,onProgress:text=>{if(pending.current===controller)setPrepareText(text);}});
