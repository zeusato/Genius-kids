...
export function DragonAdventure({student,complete,controls,onExit,onLegacy,generate}:{student:StudentProfile;complete:Completion;controls:Controls;onExit:()=>void;onLegacy:()=>void;generate?:Generate}){
 ...
 const [draft,setDraft]=useState(()=>readDraft(student.id)),[session,setSession]=useState<Session|null>(null),sessionRef=useRef(session);
 ...
 useEffect(()=>{mounted.current=true;musicManager.playTrack(MusicTrack.DRAGON_QUEST);return()=>{mounted.current=false;pending.current?.abort();speechStop.current();const s=sessionRef.current;if(s)saveDraft({...s,paused:!finished(s),questionReady:false});musicManager.resumeRouteMusic();};},[]);
 ...
 useEffect(()=>{if(!session||session.paused)return;const el=panelRef.current?.querySelector<HTMLElement>(session.phase==='feedback'?'.dq-primary':'h2');if(!el)return;if(el.tagName!=='BUTTON')el.setAttribute('tabindex','-1');el.focus({preventScroll:true});},[session?.id,session?.phase,session?.paused,q?.slotId]);
 useEffect(()=>{
 ...
 const begin=async(forceLocal=false)=>{
  ...
  try{const prepared=await prepareQuestions(config,slots,seed,{signal:controller.signal,generate:forceLocal?undefined:transport,onProgress:text=>{if(pending.current===controller)setPrepareText(text);}});
   ...
   const s=createSession(config,seed,crypto.randomUUID(),student.id,prepared);setRound(s);saveDraft(s);setDraft({session:s,corrupt:false});setModal(null);setReward(null);setSaveError(false);setMessage(prepared.notice);
 ...
 const wp={missionId:s?.config.missionId||selected,seed:s?.seed||2718,session:s||undefined,quality:prefs.quality,reduced:prefs.reduced,heroColor:prefs.heroColor,overview:true,onReady:ready,onUnavailable:unavailable,onSlow:slow};
 const fallback=<World2D {...wp}/>;
 return <div className={`dq-app ${s?'dq-playing':'dq-home'} ${prefs.reduced?'dq-reduced':''}`} style={{'--dq-sky':region.sky,'--dq-accent':region.color} as React.CSSProperties}>
  <div className="dq-shell">
   ...
   <main className="dq-main">
    <section className="dq-scene-area" aria-label="Vùng đất của rồng">
     ...
     <div className="dq-scene-footer"><span>Toàn bộ 50 ô · {prefs.renderer==='3d'?'3D':'2D'}</span><span>{s?s.prepared.source==='ai'?'Câu hỏi AI':s.prepared.source==='mixed'?'AI + câu hỏi có sẵn':'Câu hỏi có sẵn':'Gieo xúc xắc để di chuyển'}</span></div>
