...
import { Apple, Diamond, Lightbulb, Star, Volume2 } from 'lucide-react';
...
function Visual({task:t}:{task:Task}){
 if(t.kind==='count'){const Icon=t.object==='apple'?Apple:t.object==='star'?Star:Diamond;return <div className="dq-count" aria-label={`${t.left} hình và ${t.right} hình`}><span>{Array.from({length:t.left},(_,i)=><Icon aria-hidden key={i} size={28}/>)}</span>{t.right>0&&<><b>+</b><span>{Array.from({length:t.right},(_,i)=><Icon aria-hidden key={i} size={28}/>)}</span></>}</div>;}
...
export function QuestionView({session:s,onAnswer,onHint,onRead,onReady}:{session:Session;onAnswer:(value:string)=>void;onHint:()=>void;onRead:()=>void;onReady:()=>void}){
 const q=questionOf(s)!,a=attemptOf(s),feedback=s.phase==='feedback',assist=!!a?.assisted;
 ...
 return <div className="dq-question">
  <div className="dq-question-top"><span>{q.input?'Điền đáp án':'Chọn một đáp án'}</span><button className="dq-icon" aria-label="Đọc lại câu hỏi" disabled={feedback} onClick={onRead}><Volume2 size={20}/></button></div>
  <h2 id="dq-question-text">{q.text}</h2><Visual task={q.task}/>
  {!feedback&&!s.questionReady&&<button className="dq-ready" onClick={onReady}>Em đã đọc xong · Bắt đầu trả lời</button>}
  {!feedback&&s.config.mode==='challenge'&&<div className="dq-timer" aria-label={`Còn ${Math.ceil((timeLimit(s)-s.questionMs)/1000)} giây`}><span style={{width:`${Math.max(0,100-s.questionMs/timeLimit(s)*100)}%`}}/><b>{Math.max(0,Math.ceil((timeLimit(s)-s.questionMs)/1000))}s</b></div>}
