...
export function QuestionView({session:s,onAnswer,onHint,onRead,onReady}:{session:Session;onAnswer:(value:string)=>void;onHint:()=>void;onRead:()=>void;onReady:()=>void}){
 const q=questionOf(s)!,a=attemptOf(s),feedback=s.phase==='feedback',reading=s.phase==='question'&&!s.questionReady,canAnswer=s.phase==='question'&&!s.paused,assist=!!a?.assisted;
 const [value,setValue]=useState(''),input=useRef<HTMLInputElement>(null);
 useEffect(()=>{if(q.input&&canAnswer)input.current?.focus({preventScroll:true});},[q.slotId,canAnswer,q.input]);
 return <div className="dq-question">
  <div className="dq-question-top"><span>{q.input?'Điền đáp án':'Chọn một đáp án'}</span><button className="dq-icon dq-read-question" aria-label={reading?"Dừng đọc câu hỏi":"Đọc lại câu hỏi"} title={reading?"Dừng đọc câu hỏi":"Đọc lại câu hỏi"} aria-pressed={reading} disabled={s.phase!=='question'} onClick={reading?onReady:onRead}>{reading?<VolumeX size={20}/>:<Volume2 size={20}/>}</button></div>
  ...
  {q.input?<form className="dq-answer-form" onSubmit={e=>{e.preventDefault();if(canAnswer&&value.trim())onAnswer(value);}}><input ref={input} aria-label="Đáp án của em" inputMode="numeric" autoComplete="off" maxLength={7} value={value} onChange={e=>setValue(e.target.value.replace(/[^0-9]/g,''))} disabled={!canAnswer} placeholder="?"/><button className="dq-primary" disabled={!canAnswer||!value.trim()} type="submit">Trả lời</button></form>:<div className="dq-options" role="group" aria-labelledby="dq-question-text">{q.options.map((o,i)=><button key={o.id} disabled={!canAnswer} className={`dq-answer ${feedback&&o.text===q.answer?'is-correct':''} ${feedback&&!s.feedback?.correct&&s.feedback?.answer===o.id?'is-wrong':''}`} onClick={()=>onAnswer(o.id)}><span>{String.fromCharCode(65+i)}</span>{o.text}</button>)}</div>}
