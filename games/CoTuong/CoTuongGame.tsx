...
export interface CoTuongGameProps { owner?: { id: string; name: string; avatar?: string }; onExit?: () => void; complete?: (match: Match) => boolean }
...
function Game({ owner = DEFAULT_OWNER, onExit, complete }: CoTuongGameProps) {
  ...
  const [prefs, setPrefs] = useState(() => loadPrefs(owner.id)), [selected, setSelected] = useState<number | null>(null);
  const [paused, setPaused] = useState(false), [modal, setModal] = useState<Modal>(null), [motion, setMotion] = useState<Motion | null>(null);
  ...
  const { flip, setFlip, rotating } = useBoardFacing(loaded.match?.ownerSide === 1, automatic, !!motion, prefs.reduced);
  const blocked = lobby || paused || modal !== null || rotating;
  ...
  const persist = useCallback((next: Match) => {
    const draftOk = saveDraft(next);
    let resultOk = true;
    if (next.phase === 'over') {
      resultOk = persistResult(next);
      if (complete) { try { resultOk = complete(next) && resultOk; } catch { resultOk = false; } }
    }
    setSaved(draftOk && resultOk);
    if (resultOk && next.phase === 'over') setRecords(recordsFor(owner.id));
  }, [owner.id, complete]);
  ...
  }, [persist, prefs.reduced, sound]);
  useEffect(() => { if (!motion) return; const timer = setTimeout(() => setMotion(null), motion.duration + 30); return () => clearTimeout(timer); }, [motion]);
  useEffect(() => {
  ...
  function begin(replace = false) {
    ...
    const next = newMatch(owner, side, mode, level); controller.current?.cancel(); live.current = next; setMatch(next); persist(next); setFlip(turnFacing(prefs.seating, mode === 'human', next.active, true) ?? (side === 1)); setSelected(null); setMotion(null); setPaused(false); setModal(null); setLobby(false); sound('select');
  }
  ...
  const exitCategory = () => { controller.current?.cancel(); if (live.current && !saveDraft(live.current)) { setSaved(false); return; } onExit?.(); };
  return <div className={`kv-app ${prefs.reduced ? 'reduced' : ''}`}>
    <header className='kv-header'>{onExit && <BoardGamesBack onBack={exitCategory}/>}<button className='kv-brand' onClick={() => { setLobby(true); setPaused(false); setMotion(null); setSelected(null); }} aria-label='Về vườn cờ'><span className='kv-seal'>帥</span><span><strong>KỲ VIÊN</strong><small>Vườn cờ tướng</small></span></button>
    ...
    {lobby ? <main className='kv-lobby'>
      ...
      <section className='kv-setup' aria-label='Chuẩn bị ván cờ'><div className='kv-setup-title'><span className='kv-eyebrow'>CHỌN MỘT CUỘC CỜ</span><h2>Hôm nay, chơi cùng ai?</h2>{records.length > 0 && <small>{records.length} ván đã chơi · {records.filter(r => r.hostResult === 'win').length} ván thắng</small>}</div>
        <div className='kv-config'><fieldset><legend>Đối thủ</legend><div className='kv-options'><button aria-pressed={mode === 'bot'} className={mode === 'bot' ? 'active' : ''} onClick={() => setMode('bot')}><Sparkles size={18}/><span>Đấu với máy</span></button><button aria-pressed={mode === 'human'} className={mode === 'human' ? 'active' : ''} onClick={() => setMode('human')}><Users size={18}/><span>Hai người</span></button></div></fieldset>
          ...
          {mode === 'bot' ? <fieldset><legend>Mức thử thách</legend><div className='kv-levels'>{(['easy', 'medium', 'hard'] as Level[]).map(l => <button key={l} aria-pressed={level === l} className={level === l ? 'active' : ''} onClick={() => setLevel(l)}>{LEVEL_NAMES[l]}</button>)}</div></fieldset> : <SeatingChoice value={prefs.seating} onChange={value => preference('seating', value)}/>}
        </div><div className='kv-start-area'>{match?.phase === 'play' ? <><button className='kv-primary' onClick={() => { setLobby(false); setPaused(false); }}><Play size={18}/> Tiếp tục ván cờ <ArrowRight size={18}/></button><button className='kv-text-button' onClick={() => begin()}>Bắt đầu ván mới</button></> : <button className='kv-primary' onClick={() => begin()}>Bắt đầu ván cờ <ArrowRight size={19}/></button>}<small><Shield size={13}/> Ván cờ tự lưu trên máy này</small></div>
      </section><footer className='kv-lobby-footer'><span><Leaf size={14}/> Không vội vàng. Cứ tận hưởng từng nước đi.</span><button className='kv-text-button' onClick={() => setModal('rules')}>Làm quen với luật chơi <ArrowRight size={14}/></button></footer>
      ...
      <section className='kv-arena' aria-label='Ván cờ'><div className='kv-arena-top'><button className='kv-text-button' onClick={() => { setLobby(true); setMotion(null); }}><ArrowLeft size={17}/> Vườn cờ</button><div className={`kv-mobile-turn ${checked ? 'checked' : ''}`}><span aria-live='polite'>{status}</span><button className='kv-icon-button' disabled={match.phase === 'over'} aria-label={paused ? 'Tiếp tục ván cờ trên bàn' : 'Tạm dừng ván cờ trên bàn'} onClick={() => { setPaused(p => !p); setMotion(null); }}>{paused ? <Play size={16}/> : <Pause size={16}/>}</button></div><span className={`kv-save-status ${saved ? '' : 'error'}`}>{saved ? <CircleCheck size={13}/> : <AlertCircle size={13}/>} {saved ? 'Đã lưu' : 'Chưa lưu được'}</span></div>
        <div className='kv-board-stage'>
          ...
          <div className={`kv-board-canvas ${prefs.flat ? 'flat' : ''}`}>
            {prefs.flat ? <FlatBoardTurn flip={flip} reduced={prefs.reduced}><Board2D match={match} selected={selected} moves={selectedMoves} enabled={enabled} vietnamese={prefs.vietnamese || !fontReady} flip={flip} reduced={prefs.reduced} view={prefs.view} light={prefs.light} motion={motion} checked={checked} onPick={pick} onFailure={fail3D}/></FlatBoardTurn> : <SceneBoundary onFailure={fail3D}><Suspense fallback={<div className='kv-loading'><Leaf/><span>Đang bày bàn cờ…</span></div>}><Board3D match={match} selected={selected} moves={selectedMoves} enabled={enabled} vietnamese={prefs.vietnamese || !fontReady} flip={flip} reduced={prefs.reduced} view={prefs.view} light={prefs.light} motion={motion} checked={checked} onPick={pick} onFailure={fail3D}/></Suspense></SceneBoundary>}
          </div>
          ...
          {paused && match.phase === 'play' && <div className='kv-board-overlay'><div><span className='kv-pause-symbol'><Leaf/></span><h2>Nghỉ một chút nhé.</h2><p>Ván cờ đã lưu, quân cờ vẫn ở đây.</p><button className='kv-primary' onClick={() => setPaused(false)}><Play size={17}/> Tiếp tục chơi</button></div></div>}
