...
export class BotController {
  ...
  start(state: Match, moves: Move[], onResult: (move: Move, fallback: boolean, info?: BotReply) => void) {
    ...
    try {
      ...
      this.worker.postMessage({ requestId, matchId: state.id, revision: state.revision, rulesVersion: RULES_VERSION, state, level, limits: LIMITS[level], seed: state.ply + 20260915 });
