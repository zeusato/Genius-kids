    ...
    const analyzeCircuit = (comps: CircuitComponentData[], wiresData: WireData[]) => {
        ...
        wiresData.forEach(w => {
            ...
            if (isUActive && isVActive) {
                let isReverse = false;
                // Electron Flow: From Negative (Low Dist) to Positive (High Dist relative to Neg)
                const dU = distNeg.get(u) ?? 9999;
                const dV = distNeg.get(v) ?? 9999;

                // Normal: from -> to
                // If dist(from) < dist(to) => Flow is from -> to (Normal)
                if (dU < dV) {
                    isReverse = false;
                } else if (dU > dV) {
                    isReverse = true;
