                            ...
                            <div key={gameType} className="bg-gray-50 p-4 rounded-xl">
                                ...
                                <div className="grid grid-cols-3 gap-2 text-sm">
                                    ...
                                    <div>
                                        ...
                                        <p className="font-bold text-yellow-600">{gameType==='co-ti-phu'?(student.gameHistory.filter(g=>g.gameType==='co-ti-phu').reduce((sum,g)=>sum+g.score,0)/gameStats.count).toFixed(0)+' xu':gameType==='o-an-quan'?(student.gameHistory.filter(g=>g.gameType==='o-an-quan').reduce((sum,g)=>sum+g.score,0)/gameStats.count).toFixed(1)+' điểm':gameType==='horse-race'?(student.gameHistory.filter(g=>g.gameType==='horse-race').reduce((sum,g)=>sum+g.score,0)/gameStats.count).toFixed(1)+'/4':gameStats.avgStars+'⭐'}</p>
                                    </div>
                                </div>
                            </div>
                        ))}
