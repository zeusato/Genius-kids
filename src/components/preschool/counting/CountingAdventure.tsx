...
export function CountingAdventure({activity,onBack}:{activity:Activity;onBack:()=>void}) {
    ...
    const speak=async(text:string,number?:number,lang:'vi-VN'|'en-US'='vi-VN')=>{
        ...
        const result=number!==undefined?await voice.current!.number(number,text||undefined):await voice.current!.play(text,lang);
    ...
    const hear=async()=>{
        ...
        }else if(current?.activity==='learn') await speak(prompt(),current.rounds[current.index].value);
        else await speak(prompt());
    };
    useEffect(()=>{mounted.current=true;const timer=setTimeout(()=>{if(!live.current)void speak(COPY[activity].guide);},250);return()=>{mounted.current=false;clearTimeout(timer);audioRun.current++;voice.current!.cancel();};},[]);
    useEffect(()=>{
        if(s)game.current?.scrollIntoView({block:'start',behavior:'instant'});
    },[s?.id,s?.index,s?.learningStep,done]);
    useEffect(()=>{
    ...
    const begin=(resume?:Session,number=fixed)=>{
        ...
        const next=resume||makeSession(owner,activity,3,Math.max(Date.now(),(live.current?.seed??0)+1),progress,activity==='learn'?number:0);
    ...
    const chooseNumber=()=>{cancel();clockActive.current=false;setSession(null);live.current=null;};
    const continueTrip=()=>begin(undefined,activity==='learn'?round!.value%10+1:0);
    if(!s)return <section className={'nc-intro nc-'+activity}>
        ...
        <div className="nc-intro-content"><span className="nc-eyebrow">ĐẢO SỐ / {COPY[activity].subtitle}</span><h2>{COPY[activity].title}</h2><p>{COPY[activity].guide}</p><button className="nc-listen" onClick={()=>void hear()}><Volume2 size={20}/>Nghe cách chơi</button>
            ...
            {activity==='learn'?<div className="nc-number-map" aria-label="Chọn số để khám phá">{NUMBERS.map(n=><button key={n} aria-label={`Khám phá số ${n}`} aria-pressed={fixed===n} className={progress.stickers.includes(n)?'sticker':''} onClick={()=>setFixed(n)}><b>{n}</b>{progress.stickers.includes(n)?<Star size={12}/>:mastered(progress.skills.learn?.[n])?<Check size={12}/>:<span/>}</button>)}</div>:<p className="nc-range-note">Cùng chơi với các số từ <strong>1 đến 10</strong>.</p>}
    ...
    return <section ref={game} className={'nc-game nc-'+activity} aria-label={COPY[activity].title}>
        <header className="nc-game-header"><div><span className="nc-eyebrow">{COPY[activity].subtitle}</span><h2 ref={heading} tabIndex={-1}>{done?'Chuyến đi thật tuyệt!':activity==='learn'?storyFor(round!.value).title:COPY[activity].title}</h2></div><div className="nc-game-actions">
            {activity==='learn'&&<button disabled={!saved} onClick={chooseNumber}>Chọn số</button>}
            {done?<button className="nc-primary" disabled={!saved} onClick={continueTrip}>{activity==='learn'?`Số tiếp theo: ${round!.value%10+1}`:'Chơi chuyến mới'}<ArrowRight size={18}/></button>:<div className="nc-progress" aria-label={activity==='learn'?`Số ${round!.value} trong dãy 1 đến 10`:`Đã hoàn thành ${s.outcomes.length} trên ${s.rounds.length}`}><span>{activity==='learn'?`${round!.value} / 10`: `${s.outcomes.length}/${s.rounds.length}`}</span>{activity!=='learn'&&s.rounds.map((r,i)=><i key={r.id} className={i<s.outcomes.length?'complete':''}/>)}</div>}
        </div></header>
