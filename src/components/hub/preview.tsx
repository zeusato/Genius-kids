...
import { gamesFor, SCIENCE_CATALOG } from './catalog';
...
function Preview() {
    const [student, setStudent] = useState(base), [screen, setScreen] = useState(new URLSearchParams(window.location.search).get('screen') === 'science' ? 'science' : 'mode'), [last, setLast] = useState('');
    return <>
        <details style={{ font:'12px system-ui', padding:'7px 16px', background:'#193e43', color:'#f2f3df' }}>
            ...
            <div style={{ display:'flex', gap:16, flexWrap:'wrap', padding:12 }}>
                ...
                <label>Màn <select aria-label="Màn kiểm thử" value={screen} onChange={e => setScreen(e.target.value)}><option value="mode">Khám phá</option><option value="games">Trò chơi</option><option value="science">Khoa học</option></select></label><output>{last}</output>
            </div>
        </details>
