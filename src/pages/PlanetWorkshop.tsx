...
import { regenerateTown, TownHistory } from '../components/planetmaker/engine/town';
import { TownSettings } from '../components/planetmaker/TownSettings';
import { GRAPHICS } from '../components/planetmaker/rendering/environment';
...
function TownModal({ label, busy = false, onClose, children }: { label: string; busy?: boolean; onClose: () => void; children: React.ReactNode }) {
    const ref = useRef<HTMLDialogElement>(null);
    useEffect(() => { const dialog = ref.current!; dialog.showModal(); return () => dialog.close(); }, []);
    return <dialog ref={ref} className="pm-modal pm-town-dialog" aria-label={label} aria-modal="true" onCancel={e => { e.preventDefault(); if (!busy) onClose(); }}>{children}</dialog>;
}
...
function SceneRuntime({ view, phase, capture, invalidateRef, stats, cameraAction, controls }: { view: View; phase: string; capture: React.MutableRefObject<(() => Promise<Blob>) | null>; invalidateRef: React.MutableRefObject<() => void>; stats: (s: string) => void; cameraAction: React.MutableRefObject<(action: string) => void>; controls: React.MutableRefObject<any> }) {
    ...
    useFrame((_, delta) => { if (phase === 'out') { camera.position.multiplyScalar(Math.max(.92, 1 - delta * .9)); invalidate(); } elapsed.current += delta; if (elapsed.current > 1) { stats(`${gl.info.render.calls} lượt vẽ · ${gl.info.render.triangles.toLocaleString()} tam giác · ${gl.info.memory.geometries} geometry · ${gl.info.memory.textures} texture`); elapsed.current = 0; } });
    return null;
}
...
function WorldEditor({ initial }: { initial: World }) {
    ...
    const [keepBuildings, setKeepBuildings] = useState(true), [deleteTown, setDeleteTown] = useState(false);
    const townHistory = useRef(new TownHistory());
    ...
    const record = (scope: 'globe' | 'region' | 'town') => { globeRedo.current = []; history.current.redoStack = []; townHistory.current.redoStack = []; timeline.current.record(scope, { globe: globeUndo.current.length, region: history.current.undoStack.length, town: townHistory.current.undoStack.length }); };
    ...
    const travel = (redo = false) => {
        if (setup || deleteTown || generating || phase || imported) return;
        ...
        else if (scope === 'town') { const state = townHistory.current.travel(world.region, redo); if (state !== undefined) { world.region = state; setView(state ? 'region' : 'globe'); setTool('view'); setGlobeTool('rotate'); setMarkMode(false); refreshAll(); changed(); } }
    ...
    const openTerrainSetup = () => { finish(); setNotice(''); setDraft(null); setSelected(null); setKeepBuildings(true); if (world.region) { setPreset(world.region.preset); setSeedInput(String(world.region.seed)); } setSetup(true); };
    const removeTown = () => {
        finish(); if (!worldRef.current.region) return;
        townHistory.current.record(worldRef.current.region); record('town'); worldRef.current.region = null;
        setDeleteTown(false); setDraft(null); setSelected(null); setMarkMode(false); setView('globe'); setTool('view'); setGlobeTool('rotate');
        refreshAll(); changed(); void save(); setNotice('Đã xóa thị trấn. Bấm Hoàn tác để khôi phục trong phiên này.');
    };
    const createMap = () => {
        if (generating) return;
        const seed = Number(seedInput); if (!seedInput.trim() || !Number.isInteger(seed) || seed < 0 || seed > 0xffffffff) { setNotice('Mã địa hình cần là số nguyên từ 0 đến 4294967295.'); return; }
        ...
        w.onmessage = async e => {
            if (id !== workerId.current || !mounted.current) return; w.terminate(); setGenerating(false);
            if (e.data.error) { setNotice(e.data.error); return; }
            const current = worldRef.current.region;
            if (current) editRegion('Đổi địa hình thị trấn', () => Object.assign(current, regenerateTown(current, e.data.region, keepBuildings)));
            else { townHistory.current.record(null); record('town'); worldRef.current.region = e.data.region; refreshAll(); changed(); }
            setDraft(null); setSelected(null); setCursor(null); setTool('view'); setSetup(false);
            setNotice(current ? 'Đã đổi địa hình. Có thể Hoàn tác; kiểm tra lại đường và mực nước nếu giữ công trình.' : 'Đã tạo thị trấn mới.');
            if (view === 'region') { cameraAction.current('reset'); await save(); } else await switchView('region');
        }; w.postMessage({ id, preset, seed });
    };
    ...
    const acceptImport = async () => { if (!imported) return; finish(); if (!(await save())) return; backup(); imported.revision = world.revision; worldRef.current = imported; restoreSnap(terrain, imported.globe); history.current = new RegionHistory(); townHistory.current = new TownHistory(); timeline.current = new EditTimeline(); globeUndo.current = []; globeRedo.current = []; setImported(null); setDraft(null); setSelected(null); setView('globe'); refreshAll(); changed(); await save(); };
    ...
    return <main className={`pm-workshop ${view === 'region' ? 'pm-day' : ''}`}>
        ...
        <div className="pm-stage" onPointerDownCapture={canvasDown} onPointerLeave={() => { finish(); setCursor(null); setGlobeCursor(null); }}>
            {!gpuError && <SceneBoundary key={canvasKey} onError={() => setGpuError(true)}><Canvas shadows={!lowQuality && world.region?.graphics !== 'light' && view === 'region'} frameloop={!visible ? 'never' : traffic || phase ? 'always' : 'demand'} dpr={lowQuality ? 1 : [1, GRAPHICS[world.region?.graphics || 'balanced'].dpr]} camera={{ fov: 45, near: .05, far: 250 }} onCreated={({ gl }) => { gl.domElement.addEventListener('webglcontextlost', e => { e.preventDefault(); finishRef.current(true); setGpuError(true); }, { once: true }); }}>
        ...
        <aside className="pm-panel" ref={panelRef}>
            {view === 'globe' ? <>
                ...
                {tab === 'town' && town && <><div className="pm-metrics"><div><strong>{town.residents}</strong><span>Cư dân</span></div><div><strong>{region.buildings.length}/200</strong><span>Công trình</span></div><div><strong>{town.connectedHomes}</strong><span>Nhà nối đường</span></div><div><strong>{town.servedHomes}</strong><span>Nhà đủ tiện ích</span></div></div><p className="pm-help">Mỗi mạng đường có điện và nước riêng. 1 trạm mặt trời cấp 12, trạm điện gió cấp 18 đơn vị; 1 trạm nước có điện cấp 20 đơn vị. Cả mạng cần đủ nguồn cho tổng nhu cầu.</p><div className="pm-networks">{town.networks.filter(n => n.powerNeed || n.waterNeed || n.power).map((n, i) => <p key={i}>Mạng {i + 1}: ⚡ {n.power}/{n.powerNeed} · 💧 {n.power >= n.powerNeed ? n.water : 0}/{n.waterNeed} <small>(nguồn / nhu cầu)</small></p>)}</div><h3>Ba thử thách nhỏ</h3>{['Nối đường cho 5 ngôi nhà', 'Cấp đủ điện, nước cho 3 ngôi nhà', 'Nối trường học và công viên với khu dân cư'].map((m, i) => <div className={`pm-mission ${town.missions[i] ? 'done' : ''}`} key={m}>{town.missions[i] ? '✓' : '○'} {m}</div>)}<p className="pm-help">Chơi tự do, không cần hoàn thành thử thách. Cư dân và xe là minh họa.</p><h3>Thí nghiệm nước dâng</h3><label className="pm-slider">Mực nước vùng <output>{region.seaLevel.toFixed(1)}</output><input aria-label="Mực nước vùng" type="range" min="-2" max="6" step=".1" value={region.seaLevel} onChange={e => editRegion('Nước dâng', () => { region.seaLevel = +e.target.value; })} /></label><p className="pm-help">{town.flooded ? `${town.flooded} công trình đang ngập. Đường ngập ngừng kết nối.` : 'Chưa có công trình bị ngập.'} Bấm Hoàn tác để quay lại.</p></>}
                {tab === 'settings' && <><div className="pm-row"><button disabled={!!phase} onClick={openTerrainSetup}>Đổi địa hình thị trấn</button><button className="pm-danger" disabled={!!phase} onClick={() => { finish(); setDraft(null); setSelected(null); setDeleteTown(true); }}>Xóa thị trấn</button></div><p className="pm-help">{PRESETS[region.preset]} · Mã {region.seed}<br />Bản đồ 128 × 128 ô địa hình, 64 × 64 ô xây dựng. Thay đổi trên hành tinh không làm đổi bản đồ này.</p><label className="pm-check"><input type="checkbox" checked={traffic} onChange={e => setTraffic(e.target.checked)} />Cư dân đi bộ và xe minh họa</label><TownSettings region={region} fallback={lowQuality} onLimits={limits => editRegion('Đổi giới hạn thị trấn', () => { region.limits = limits; })} onGraphics={quality => { setLowQuality(false); editRegion('Đổi chất lượng hình ảnh', () => { region.graphics = quality; }); }} /><details><summary>Thông tin hiển thị</summary><small>{stats}</small></details></>}
        ...
        {setup && <TownModal label={region ? 'Đổi địa hình thị trấn' : 'Tạo vùng xây dựng'} busy={generating} onClose={() => setSetup(false)}>
            <span className="pm-modal-icon">🏡</span><h2>{region ? 'Đổi địa hình thị trấn' : 'Chọn nơi thị trấn bắt đầu'}</h2>
            <p>{region ? 'Chọn loại và mã để tạo lại toàn bộ đất, cây và mực nước. Tên thị trấn và dấu mốc trên hành tinh được giữ nguyên.' : 'Một vùng riêng để nặn đất, làm đường và xây nhà. Hành tinh sẽ có dấu mốc dẫn đến đây.'}</p>
            <div className="pm-preset-grid">{Object.entries(PRESETS).map(([id, name]) => <button key={id} aria-pressed={preset === id} className={preset === id ? 'active' : ''} disabled={generating} onClick={() => setPreset(id as Preset)}>{name}</button>)}</div>
            <label>Mã địa hình<input aria-label="Mã địa hình" disabled={generating} type="number" min="0" max="4294967295" step="1" value={seedInput} onChange={e => setSeedInput(e.target.value)} /></label>
            {region && <><label className="pm-check"><input type="checkbox" disabled={generating} checked={keepBuildings} onChange={e => setKeepBuildings(e.target.checked)} />Giữ công trình và đường</label><p>{keepBuildings ? 'Giữ vị trí và nền nhà; đường đi theo địa hình mới nên có thể bị ngập. Hãy kiểm tra kết nối sau khi đổi.' : 'Sẽ xóa ' + region.buildings.length + ' công trình và toàn bộ đường để bắt đầu lại.'} Có thể Hoàn tác trong phiên này.</p></>}
            <p className="pm-help">Cùng loại và mã tạo cùng địa hình ban đầu. Các nét nặn đất trước đó sẽ được thay thế.</p>
            {notice && <p role="alert">{notice}</p>}
            <div className="pm-row"><button className="pm-primary" disabled={generating} onClick={createMap}>{generating ? 'Đang tạo địa hình…' : region ? 'Xác nhận đổi địa hình' : 'Tạo và ghé thăm →'}</button><button disabled={generating} onClick={() => setSetup(false)}>Hủy</button></div>
        </TownModal>}
        {deleteTown && region && <TownModal label="Xóa thị trấn" onClose={() => setDeleteTown(false)}><h2>Xóa “{region.name}”?</h2><p>Sẽ xóa toàn bộ địa hình, {region.buildings.length} công trình, đường và cây của thị trấn. Hành tinh vẫn được giữ nguyên.</p><p>Có thể bấm Hoàn tác để khôi phục trước khi rời hoặc tải lại Xưởng. Xuất bản sao nếu muốn giữ thị trấn lâu dài.</p><div className="pm-row"><button onClick={backup}>Xuất bản sao</button><button className="pm-danger" onClick={removeTown}>Xác nhận xóa thị trấn</button><button autoFocus onClick={() => setDeleteTown(false)}>Hủy</button></div></TownModal>}
